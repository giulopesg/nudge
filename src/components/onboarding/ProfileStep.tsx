...
export default function ProfileStep({ neurotags, goals = [], onNext }: Props) {
  ...
  return (
    <div className="flex flex-col items-center px-4">
      ...
      {/* Goals chosen */}
      {goals.length > 0 && (
        <div className="mt-4 w-full max-w-md card chamfer-md border-primary/20 bg-primary-muted/30">
          <h3 className="font-mono text-[10px] font-semibold uppercase tracking-[0.15em] text-primary">
            {tGoals('profileGoalsTitle')}
          </h3>
          <p className="mt-1 text-xs text-text-muted">{tGoals('profileGoalSelected')}</p>
          <ul className="mt-2 space-y-1">
            {goals.map((goalId) => (
              <li key={goalId} className="flex items-start gap-2 text-sm text-foreground">
      ...
      <div className="mt-4 w-full max-w-md space-y-2">
        ...
        {visibleTraits.map((tag) => (
          <div key={tag} className="card chamfer-sm p-4">
            <span className="font-mono text-[10px] font-semibold text-plum-light tracking-wider">
      ...
      {/* Nudge actions */}
      <div className="mt-4 w-full max-w-md card chamfer-md border-primary/20 bg-primary-muted/50">
        <h3 className="font-mono text-[10px] font-semibold uppercase tracking-[0.15em] text-primary">
          {t('profile.nudgeWill')}
        </h3>
